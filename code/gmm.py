...
def locational_demand_analysis(park_data, gps_loc):
    ...
    func = functools.partial(GMM_new, park_data, gps_loc, times)
...
def GMM_new(park_data, gps_loc, times, iteration):
    ...
    for train_time in data_df.index.get_level_values(0).unique().tolist():
        ...
        I = moran_auto.moran_mixture(train_loads, train_labels, N)
        expectation = moran_auto.moran_expectation(N)
        variance = moran_auto.moran_variance(train_loads, weights, N)
    ...
    return time_avg_error, moran_mixture
