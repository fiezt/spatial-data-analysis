...
def locational_demand_analysis_gmm(park_data, gps_loc, num_comps, k, verbose=True):
    ...
    func = functools.partial(locational_demand_one_time_gmm, park_data, gps_loc,
                             times, num_comps, k, verbose)
    results = pool.map(func, range(len(times)))
...
def locational_demand_one_time_gmm(park_data, gps_loc, times, num_comps, k, verbose,
                               iteration):
    """Find GMM consistency and spatial autocorrelation at one day of the week and time of day.

    I took out the moran stuff in this one.
    """
    ...
    # Getting the data for the current day and hour combination.
    data_df = park_data.loc[
        (park_data['Day'] == day) & (park_data['Hour'] == hour)]
    block_keys = sorted(data_df.index.get_level_values(1).unique().tolist())
    ...
    for train_time in data_df.index.get_level_values(0).unique().tolist():
        ...
        train_active_index = train_data.index.tolist()
        train_mask = [block_keys.index(train_active_index[i]) for i in
                      xrange(len(train_active_index))]
        train_to_label_map = {train_active_index[i]: i for i in
                              xrange(len(train_active_index))}
        ...
        # Scaling the mean back to GPS coordinates and saving the centroids.
        means = np.vstack(([(mean[1:] - scaler.min_[1:]) / (scaler.scale_[1:])
        ...
        for test_time in data_df.index.get_level_values(0).unique().tolist():
            ...
            test_data = test_data.loc[test_data.index.isin(train_active_index)]
            test_active_index = test_data.index.tolist()
            test_mask = [block_keys.index(test_active_index[i]) for i in
                         xrange(len(test_active_index))]
    ...
    # Average consistency for the particular day and hour combination.
    time_avg_consistency = round(np.array(average_consistencies).mean() * 100,
                                 2)

    if verbose:
        print('Finished day %d and hour %d' % (day, hour))

    return day, hour, time_avg_consistency, centers


def locational_demand_analysis_moran(park_data, gps_loc, num_comps, k, verbose=True):
    ...
    func = functools.partial(locational_demand_one_time_moran, park_data, gps_loc,
                             times, num_comps, k, verbose)
    results = pool.map(func, range(len(times)))
...
def locational_demand_one_time_moran(park_data, gps_loc, times, num_comps, k, verbose,
                               iteration):
    """Find GMM consistency and spatial autocorrelation at one day of the week and time of day.

    I took out the consistency stuff in this one.
    """
    ...
    for train_time in data_df.index.get_level_values(0).unique().tolist():
        ...
        morans_mixture.append([I, expectation, variance, z_score, p_one_sided, p_two_sided])
       # morans_mixture.append([.1, .02, .03, .04, .001, .0001])
        ...
        morans_area.append([I, expectation, variance, z_score, p_one_sided, p_two_sided])
       # morans_area.append([.1, .02, .03, .04, .001, .0001])

        # Getting spatial correlation statistics for Moran's I using nearest neighbor connections.
        # weights = moran_auto.get_neighbor_weights(gps_loc[train_mask], N, k)
        # I = moran_auto.moran_neighbor(train_loads[:, 0], gps_loc[train_mask], N,
        #                               k)
        # expectation = moran_auto.moran_expectation(N)
        # variance = moran_auto.moran_variance(train_loads[:, 0], weights, N)
        # z_score = moran_auto.z_score(I, expectation, variance)
        ...
        morans_neighbor.append([.1, .02, .03, .04, .001, .0001])
    ...
    return day, hour, morans_mixture, morans_area, morans_neighbor, centers
